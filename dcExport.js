...
class DCExport {
  ...
  async makeCsv() {
    ...
    return convertArrayToCSV(data, {header: ['work_accession_number', 'accession_number', 'filename', 'description', 'role']});
  }
  ...
  async fetchImageInfo(image) {
    ...
    return Promise.all(
      image._source.member_ids.map(async (fileSetId, index) => {
        ...
        return [workAccessionNumber, `${workAccessionNumber}_FILE_${index}`, `${this.collectionId}/${info.label}`, info.label, 'AM']
      })
    );
  }
}
