...
class DCExport {
  constructor(name, query, size) {
    console.log(name, query, size);
    this.name = name;
    try {
      this.query = JSON.parse(query);
    } catch {
      this.query = { "collection.id": query };
    }
    this.size = size;
  }
  ...
  async makeCsv() {
    ...
    fs.writeFileSync(`${this.name}.csv`, csv);
  ...
  async fetchCollectionData() {
    ...
    let result = Promise.all(
      ...
      .then((result) => result.flat().filter((v) => v));
    return result;
  }
  ...
  async fetchImageInfo(image) {
    ...
    return Promise.all(
      image._source.member_ids.map(async (fileSetId, index) => {
        ...
        let imageFilename = `${this.name}/${info.label.replace(".tif", ".jpg")}`;
...
module.exports = async (...params) => {
  let exporter = new DCExport(...params);
